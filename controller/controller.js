...
const index = ('/', function (req, res) {
    //query
    connection.query('SELECT * FROM card_anggota ORDER BY id desc', 
    function (err, rows) {
        if (err) {
            ...
            return res.status(200).json({
                status: true,
                message: 'Anda melakukannya dengan baik sehingga dapat mengakses tabel card_anggota',
                data: rows
            })
        }
    });
...
function showAnggota (req, res) {
    ...
    connection.query(`SELECT * FROM card_anggota WHERE id = ${id}`, function (err, rows) {
...
function tambahAnggota (req, res) {
    ...
    // insert query
    connection.query('INSERT INTO card_anggota SET ?', formData, function (err, rows) {
        //if(err) throw err
        if (err) {
            return res.status(500).json({
                status: false,
...
function updateAnggota (req, res) {
    ...
    // update query
    connection.query(`UPDATE card_anggota SET ? WHERE id = ${id}`, formData, function (err, rows) {
        //if(err) throw err
        if (err) {
            return res.status(500).json({
                status: false,
